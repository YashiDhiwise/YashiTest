...
beforeAll(async function (){
  try {
    ...
    const db = client.db('Express-cart_test');

    const user = db.collection('user');
    inserted_user = await user.insertOne({
      username: 'Gussie11',
      password: 'bRqLGqhHeN1f7MH',
      email: 'Magali44@gmail.com',
      name: 'Luke Altenwerth',
      role: 760,
      resetPasswordLink: {},
      loginRetryLimit: 134,
      loginReactiveTime: '2022-06-06T23:49:13.039Z',
      id: '618deda635e5d803d54352f5'
    });
  }
  catch (err) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Kailyn.Wolff31',
        'password':'TGlE8VIQ3H06TLw',
        'email':'Jaquelin.Vandervort52@hotmail.com',
        'name':'Barry Bins',
...
describe('POST /login -> if username and password is correct', () => {
  test('should return user with authentication token', async () => {
    let user = await request(app)
      ...
      .send(
        {
          username: 'Jaquelin.Vandervort52@hotmail.com',
          password: 'TGlE8VIQ3H06TLw'
        }
      );
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    ...
    let user = await request(app)
      .post('/admin/auth/forgot-password')
      .send({ 'email':'Jaquelin.Vandervort52@hotmail.com', });
